...
const RULES = {
  ...
  style: ["visible halftone dot patterns in mid-tones", "ink bleed and slight registration misalignment", "limited palette of 3-5 spot colors", "bold graphic simplified shapes", "worn and slightly imperfect print texture"],
  composition: ["bold poster-style composition", "strong central graphic element", "layered ink impression feel", "print imperfections near edges are authentic"],
  color: ["limited spot color palette — maximum 5 colors", "ink-on-paper color mixing where overlaps occur", "slightly desaturated retro tones", "visible paper texture through thin ink areas"],
  quality: ["authentic screen print reproduction quality", "visible ink texture and halftone dots", "professional vintage poster finish", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["photorealism", "smooth digital gradients", "more than 5 colors", "any text, letters, or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-screenprint error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});