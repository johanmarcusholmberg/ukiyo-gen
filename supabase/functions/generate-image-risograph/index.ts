...
const RULES = {
  ...
  style: ["visible grain from ink drum texture", "layered spot colors with overlap creating new tones", "slight registration misalignment between color layers", "bold simplified graphic forms", "paper texture visible through ink"],
  composition: ["bold graphic composition suited for poster format", "simplified forms with clear silhouettes", "layered color planes creating depth", "grain and misregistration at edges are authentic"],
  color: ["limited spot color palette — 2-4 riso ink colors", "color overlap creates mixed tones naturally", "fluorescent or soy-based ink color feel", "warm paper base visible in light areas"],
  quality: ["authentic risograph texture and grain", "professional indie print quality", "visible ink layering detail", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["photorealism", "smooth digital rendering", "complex detailed rendering", "any text or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-risograph error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});