...
const RULES = {
  ...
  composition: ["poster-style bold layout", "strong graphic presence", "print imperfections at edges are authentic"],
  color: ["limited spot color palette", "slightly desaturated retro tones"],
  quality: ["authentic print texture quality", "visible ink and halftone detail", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-screenprint-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});