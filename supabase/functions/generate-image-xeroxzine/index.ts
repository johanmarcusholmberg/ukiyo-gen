...
const RULES = {
  ...
  style: ["harsh photocopy contrast — crushed blacks and blown whites", "visible copier noise, grain, and artifacts", "collage cut-and-paste energy", "rough unpolished DIY feel", "slightly skewed or imperfect alignment"],
  composition: ["raw collage-style layout", "cut-and-paste layered elements", "intentional imperfection and asymmetry", "copier artifacts at edges are authentic"],
  color: ["strictly black and white — photocopy monochrome", "harsh contrast with lost mid-tones", "copier grain as texture"],
  quality: ["authentic photocopy reproduction quality", "visible copier noise and artifacts", "intentional lo-fi print aesthetic", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["any color", "clean digital rendering", "polished professional look", "any readable text or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-xeroxzine error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});