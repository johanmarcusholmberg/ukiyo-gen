...
const RULES = {
  ...
  style: ["heavy bold graphic shapes and stark contrasts", "raw unrefined design energy", "large bold masses of black and color", "grid-breaking asymmetric layout", "industrial print aesthetic"],
  composition: ["bold asymmetric graphic layout", "heavy visual weight distribution", "dramatic scale contrasts", "raw edges and bold shapes are intentional design"],
  color: ["stark high-contrast palette — often black + 1-2 accent colors", "no subtle tones — everything bold and uncompromising", "industrial color feeling"],
  quality: ["crisp bold graphic edges", "professional print design quality", "intentional rawness in execution", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["photorealism", "soft or pretty aesthetics", "decorative ornament", "any text, typography, or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-brutalistposter error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});