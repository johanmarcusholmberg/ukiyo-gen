...
const RULES = {
  ...
  style: ["bold ink outlines with halftone shading", "vintage four-color process palette", "action-energy composition"],
  composition: ["dynamic graphic composition", "strong figure-ground separation", "bold outlines at edges are intentional"],
  color: ["vintage CMYK comic palette", "halftone dot patterns"],
  quality: ["crisp outlines and consistent halftones", "professional comic print quality", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-retrocomic-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});