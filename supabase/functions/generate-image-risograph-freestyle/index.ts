...
const RULES = {
  ...
  style: ["grainy ink texture with layered spot colors", "slight misregistration between layers", "bold graphic simplification"],
  composition: ["bold poster-style layout", "simplified graphic forms", "grain at edges is authentic"],
  color: ["limited spot color palette", "overlap mixing creates tones"],
  quality: ["authentic riso print quality", "visible grain and layering", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-risograph-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});