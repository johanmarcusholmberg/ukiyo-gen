...
const RULES = {
  ...
  style: ["bold consistent black outlines — thicker than normal illustration", "flat solid color fills — no gradients within shapes", "classic American traditional tattoo vocabulary", "symbolic graphic icon composition", "slight vintage paper aging feel"],
  composition: ["centered iconic presentation like a flash sheet", "clean graphic isolation of the subject", "symmetry and balance in design", "bold outlines at edges are part of the tattoo design"],
  color: ["limited traditional tattoo palette: red, green, yellow, blue, black", "flat solid fills with no blending", "warm cream or aged paper background", "bold color contrast against thick outlines"],
  quality: ["crisp bold outlines with consistent weight", "clean flat color fills", "professional tattoo flash quality", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["photorealism", "soft shading or gradients within shapes", "modern tattoo realism style", "any text, banners with words, or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-tattooflash error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});