...
const RULES = {
  ...
  style: ["bold thick black outlines", "flat solid color fills", "graphic icon-style composition"],
  composition: ["centered iconic presentation", "clean graphic isolation", "bold outlines at edges are intentional"],
  color: ["limited traditional tattoo colors", "flat fills without gradients"],
  quality: ["crisp bold outlines", "clean flat fills", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-tattooflash-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});