...
const RULES = {
  ...
  style: ["heavy bold graphic shapes", "stark contrast and raw energy", "industrial design aesthetic"],
  composition: ["bold asymmetric layout", "dramatic scale and weight", "raw edges are intentional"],
  color: ["high-contrast limited palette", "bold uncompromising tones"],
  quality: ["crisp graphic edges", "professional print quality", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["soft pretty aesthetics", "decorative ornament", "any text or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-brutalistposter-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});