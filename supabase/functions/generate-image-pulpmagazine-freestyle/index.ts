...
const RULES = {
  ...
  style: ["dramatic painted illustration with rich rendering", "strong chiaroscuro lighting", "vintage mid-century aesthetic"],
  composition: ["dramatic cinematic composition", "strong central subject", "painted details at edges must be preserved"],
  color: ["rich saturated vintage palette", "dramatic lighting contrasts"],
  quality: ["professional painted illustration quality", "visible paint texture", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-pulpmagazine-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});