...
const RULES = {
  ...
  style: ["bold black ink outlines with consistent weight", "halftone dot patterns for shading and color", "vintage four-color process comic palette", "action-oriented dynamic energy", "slightly aged paper color treatment"],
  composition: ["dynamic action-oriented composition", "strong figure-ground separation", "dramatic perspective and foreshortening", "panel-like framing energy", "bold outlines at edges are intentional comic framing"],
  color: ["vintage CMYK four-color process palette", "halftone dots for mid-tones and shadows", "slightly warm aged-paper base", "bold primary and secondary colors"],
  quality: ["crisp bold ink outlines", "consistent halftone dot pattern", "professional vintage comic print quality", "high detail", "sharp focus", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["photorealism", "soft shading or smooth gradients", "modern digital comic style", "any readable text, speech bubbles, or script"],
};
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-retrocomic error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});