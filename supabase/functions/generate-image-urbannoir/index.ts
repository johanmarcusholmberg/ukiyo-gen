...
const RULES = {
  ...
  style: ["strictly monochrome — black, white, and grey only", "heavy film grain texture throughout", "high contrast with deep blacks and blown-out whites", "raw unpolished documentary aesthetic", "analog camera flash harshness when appropriate"],
  composition: ["urban street-level perspective", "dynamic framing with gritty energy", "subject fills frame with presence", "all edge details and grain textures must be preserved fully"],
  color: ["strictly black and white — no color whatsoever", "full tonal range from pure black to pure white", "grain and noise as textural elements"],
  quality: ["authentic analog film grain quality", "sharp detail in focus areas", "professional print-ready monochrome", "high detail", "sharp focus", "clean edges", "high resolution", "detailed textures", "print-ready resolution", "suitable for large format printing"],
  avoid: ["any color or tinted tones", "clean digital look", "soft or dreamy aesthetics", "any text, watermarks, or script"],
};

function buildPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "Use a warm cream/off-white vintage paper background tone as OUTER presentation." : "The background MUST be pure white (#FFFFFF) as OUTER presentation.";
...
serve(async (req) => {
  ...
  } catch (e) { console.error(`generate-image-${STYLE_KEY} error:`, e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});
